...
public class RentACatTest {
	...
	public void testListCatsNumCats3() {
		...
		r.addCat(c3);
		for (int i = 1; i <= 3; i++) {
			Mockito.when(r.getCat(i).getId()).thenReturn(i);
		}
		Mockito.when(r.getCat(1).getName()).thenReturn("Jennyanydots");
		Mockito.when(r.getCat(2).getName()).thenReturn("Old Deuteronomy");
		Mockito.when(r.getCat(3).getName()).thenReturn("Mistoffelees");
